...
find_function(const char *const fname) {
    ...
    int res = address_by_fname(&addrs, fname, &faddr);
    if (!res) {
        return faddr;
    }

    res = naive_address_by_fname(&addrs, fname, &faddr);
    if (!res) {
        return faddr;
    }

    struct Elf64_Sym *stab_s = (struct Elf64_Sym *)uefi_lp->SymbolTableStart;
    struct Elf64_Sym *stab_e = (struct Elf64_Sym *)uefi_lp->SymbolTableEnd;
    char *strtab = (char *)uefi_lp->StringTableStart;

    for (; stab_s != stab_e; stab_s++) {
        if ((ELF64_ST_BIND(stab_s->st_info) == STB_GLOBAL) && (ELF64_ST_TYPE(stab_s->st_info) == STT_FUNC)) {
            const char *sname = strtab + stab_s->st_name;
            if (!strcmp(fname, sname)) {
                return stab_s->st_value;
            }
        }
    }
