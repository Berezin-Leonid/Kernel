...
timer_start(const char *name) {
    ...
    // Ищем таймер по имени
    for (int i = 0; i < MAX_TIMERS; i++) {
        if (strcmp(timertab[i].timer_name, name) == 0) {
            timer_id = i;
            timer_started = true;
            timer = read_tsc();                // Запоминаем текущее значение TSC
            freq = timertab[i].get_cpu_freq(); // Получаем частоту процессора
            return;
        }
    }
    print_timer_error(); // Ошибка, если таймер не найден
}

void
timer_stop(void) {
    if (!timer_started) {
        print_timer_error(); // Ошибка, если таймер не был запущен
        return;
    }

    uint64_t elapsed_ticks = read_tsc() - timer; // Прошедшие тики TSC
    timer_started = false;
    timer_id = -1;

    if (freq > 0) {
        unsigned seconds = elapsed_ticks / freq; // Перевод в секунды
        print_time(seconds);
    } else {
        print_timer_error(); // Ошибка, если частота не была установлена
    }
}
    ...
    for (int i = 0; i < MAX_TIMERS; i++) {
        if (strcmp(timertab[i].timer_name, name) == 0) {
            ...
            if (freq > 0) {
                ...
                print_timer_error(); // Ошибка, если частота не получена
            }
            return;
        }
    }
