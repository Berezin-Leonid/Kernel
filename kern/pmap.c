...
alloc_child(struct Page *parent, bool right) {
    ...
    new->addr = right ? parent->addr + offset : parent->addr;
...
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    ...
    while (start < end) {
        size_t class = MAX_CLASS;
        uintptr_t step = CLASS_SIZE(class);

        // class we need
        while (step > (end - start) || (start & CLASS_MASK(class))) {
            class --;
            step = CLASS_SIZE(class);
        }

        // make/allock new class
        page_lookup(NULL, start, class, type, true);
...
dump_memory_lists(void) {
    ...
    for (size_t i = 0; i < MAX_CLASS; i++) {
        ...
        while (node != list) {
            ...
            cprintf("%016lx - %016llx \n", page2pa(page), page2pa(page) + CLASS_MASK(i));
            node = node->next;
        }
    }
}
...
init_memory(void) {
    ...
    if (res < 0) panic("Failed to map physical memory to kernel space: %d\n", res);
    ...
    res = map_physical_region(&kspace, (uintptr_t)__text_start, PADDR(__text_start), ROUNDUP(__text_end - __text_start, CLASS_SIZE(0)), PROT_R | PROT_X);
    if (res < 0) panic("Failed to map .text section: %d\n", res);
    ...
    for (; mstart < mend; mstart = (void *)((uint8_t *)mstart + uefi_lp->MemoryMapDescriptorSize)) {
        if (mstart->Attribute & EFI_MEMORY_RUNTIME) {
            ...
            if (res < 0) panic("Failed to map UEFI runtime memory: %d\n", res);
        }
    }
    ...
    if (res < 0) panic("Failed to map framebuffer: %d\n", res);
    ...
    if (res < 0) panic("Failed to map low physical memory: %d\n", res);

    res = map_physical_region(&kspace, X86ADDR((uintptr_t)__text_start), PADDR(__text_start), ROUNDUP(__text_end - __text_start, CLASS_SIZE(0)), PROT_R | PROT_X);
    if (res < 0) panic("Failed to map kernel .text section: %d\n", res);
